package br.community.feature.user.deletion;
...
public final class Deletions {
     ...
     *  para o recurso, ou {@code MOVE} sem {@code targetId} → {@link DomainError.Validation} (422). */
    public static Result<@Nullable DeletionStrategy, DomainError> parse(
        ...
        try {
            ...
            return Result.failure(new DomainError.Validation("Estratégia de exclusão inválida: " + raw));
        }

        if (!allowed.contains(strategy)) {
            return Result.failure(new DomainError.Validation("Estratégia de exclusão não suportada para este recurso: " + strategy));
        }
        if (strategy == DeletionStrategy.MOVE && targetId == null) {
            return Result.failure(new DomainError.Validation("Estratégia MOVE requer targetId"));
        }

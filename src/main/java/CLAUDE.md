...
* **Injeção de dependência (modelo híbrido):** o contexto é montado por um *composition root* (`MonetaryBootstrap`/`PeopleBootstrap`) via `br.commons.Registry` (service locator manual) — **sem Spring no contexto** (garantido pela regra ArchUnit `context_must_not_depend_on_spring`). A camada `feature/*` permanece em Spring e alcança o contexto **apenas via Facade**, reexposta como bean por `br.community.core.ContextBridge` (única costura Spring↔Registry).

**Persistência (JDBC/H2):** as portas de repositório são implementadas por adaptadores `*JDBCRepository` em `br.community.infra.persistence`, sobre o `DataSource` H2 (in-memory) criado em `DatabaseConfig` e publicado no `Registry`. O schema vive em `Database` (tabelas planas; enums como string referenciando tabelas de domínio/lookup `MON_ACCOUNT_TYPE`/`MON_NATURE`/`MON_TRANSACTION_STATUS` via FK; mapas livres como `additionalInfo`/`preferences` em coluna JSON; sem outras chaves estrangeiras entre tabelas de dados). Os primitivos JDBC ficam em `br.commons.framework.persistence.jdbc`. (A feature `Closing` ainda usa JSON via `Storage`/`LocalFileStorage`, assim como o catálogo global de centros de custo.)
...
* **Testes de Arquitetura (ArchUnit):** O arquivo `br.community.ArchitectureTest` automatiza a validação das regras arquiteturais no pipeline. As regras principais validadas são:
  ...
  6. `context_must_not_depend_on_spring`: Garante que o contexto é livre de framework (DI via `Registry`; Spring fica na borda `feature`/`core`).

...
(function () {
  ...
  function openImportModal() {
    ...
    const bodyHtml =
      '<div class="form-group">' +
        ...
        '<p style="font-size:12px;color:var(--text-muted);margin-top:8px;">' +
          'Envie o PDF original da fatura do cartão. Bancos suportados: Santander e BTG.' +
        '</p>' +
    ...
    function previewRowHtml(row, idx) {
      ...
      return (
        '<tr style="border-top:1px solid var(--border);' + (dup ? 'opacity:0.6;' : '') + '">' +
          ...
          '<td style="padding:8px 10px;">' + esc(row.description) +
        ...
        ? '<div style="max-height:360px;overflow:auto;border:1px solid var(--border);border-radius:var(--radius-sm);margin-top:12px;">' +
            '<table style="width:100%;border-collapse:collapse;font-size:13px;">' +
              '<thead><tr style="position:sticky;top:0;background:var(--bg-hover);">' +
                ...
                '<th style="padding:8px 10px;width:90px;text-align:left;font-size:11px;color:var(--text-muted);">Data</th>' +
                '<th style="padding:8px 10px;text-align:left;font-size:11px;color:var(--text-muted);">Categoria</th>' +
                '<th style="padding:8px 10px;text-align:center;font-size:11px;color:var(--text-muted);">Parcela</th>' +
                '<th style="padding:8px 10px;text-align:left;font-size:11px;color:var(--text-muted);">Descrição</th>' +
    ...
    function showConfirmSummary(created, skipped) {
      m.$body.html(
        '<div style="display:flex;align-items:center;gap:12px;padding:8px 0;">' +
          '<span style="color:var(--income);display:flex;">' + window.icon('check', 22) + '</span>' +
          '<div>' +
            ...
            '<p style="font-size:13px;color:var(--text-muted);margin-top:2px;">' +
              esc(created + ' criados, ' + skipped + ' ignorados') +
            '</p>' +
          '</div>' +
        '</div>'
      );
    ...
    m.$el.on('click', '[data-act=do-import]', function () {
      ...
      window.App.TransactionService.importPreview(selectedFile, password || null)
        .then(function (preview) { showPreview(preview); })
        .catch(function (err) {
          $btn.prop('disabled', false);

...
class CreditCardUseCaseTest {
    ...
    private CreditCard createCard(UUID accountId, String last4) {
        return ((Result.Success<CreditCard, DomainError>) useCase.createCard(new CardCommand(accountId, last4))).value();
    }
    ...
    void createsCard() {
        ...
        Result<CreditCard, DomainError> r = useCase.createCard(new CardCommand(account.id(), "1234"));
        assertTrue(r.isSuccess());
        CreditCard creditCard = ((Result.Success<CreditCard, DomainError>) r).value();
    ...
    void rejectsMalformedLast4() {
        Account account = seedChecking();
        Result<CreditCard, DomainError> r = useCase.createCard(new CardCommand(account.id(), "12a4"));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.Validation.class, ((Result.Failure<CreditCard, DomainError>) r).error());
    }

    @Test
    @DisplayName("conta inexistente → NotFound")
    void rejectsUnknownAccount() {
        Result<CreditCard, DomainError> r = useCase.createCard(new CardCommand(UUID.randomUUID(), "1234"));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.NotFound.class, ((Result.Failure<CreditCard, DomainError>) r).error());
    }
    ...
    void rejectsInactiveAccount() {
        ...
        Result<CreditCard, DomainError> r = useCase.createCard(new CardCommand(inactive.id(), "1234"));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.BusinessRule.class, ((Result.Failure<CreditCard, DomainError>) r).error());
    }
    ...
    void rejectsDuplicateLast4OnSameAccount() {
        ...
        Result<CreditCard, DomainError> r = useCase.createCard(new CardCommand(account.id(), "1234"));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.Conflict.class, ((Result.Failure<CreditCard, DomainError>) r).error());
    }
    ...
    void blockDeletesCardWithoutTransactions() {
        Account account = seedChecking();
        CreditCard creditCard = createCard(account.id(), "1234");
        Result<List<UUID>, DomainError> r = useCase.deleteCard(creditCard.id(), new TransactionPolicy.Block());
        assertTrue(r.isSuccess());
        assertTrue(((Result.Success<List<CreditCard>, DomainError>) useCase.listCardsByAccount(account.id())).value().isEmpty());
    }
    ...
    void blockRejectsCardWithLinkedTransaction() {
        ...
        assertTrue(cardRepo.findById(creditCard.id()).isPresent());
    }
    ...
    void moveMigratesTransactionsToTargetCard() {
        Account account = seedChecking();
        CreditCard source = createCard(account.id(), "1234");
        CreditCard target = createCard(account.id(), "5678");
    ...
    void moveRejectsSelfTarget() {
        Account account = seedChecking();
        CreditCard creditCard = createCard(account.id(), "1234");
        Result<List<UUID>, DomainError> r = useCase.deleteCard(creditCard.id(), new TransactionPolicy.Move(creditCard.id()));
        assertTrue(r.isFailure());
    ...
    void moveRejectsUnknownTarget() {
        Account account = seedChecking();
        CreditCard creditCard = createCard(account.id(), "1234");
        Result<List<UUID>, DomainError> r = useCase.deleteCard(creditCard.id(), new TransactionPolicy.Move(UUID.randomUUID()));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.NotFound.class, ((Result.Failure<List<UUID>, DomainError>) r).error());
    }
    ...
    void moveRejectsTargetFromAnotherAccount() {
        Account a = seedChecking();
        Account b = seedChecking();
        CreditCard source = createCard(a.id(), "1234");
        CreditCard target = createCard(b.id(), "5678");
        Result<List<UUID>, DomainError> r = useCase.deleteCard(source.id(), new TransactionPolicy.Move(target.id()));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.BusinessRule.class, ((Result.Failure<List<UUID>, DomainError>) r).error());
    }
    ...
    void purgeDeletesCardAndItsTransactions() {
        ...
        Result<List<UUID>, DomainError> r = useCase.deleteCard(creditCard.id(), new TransactionPolicy.Purge());

        assertTrue(r.isSuccess());
        assertEquals(List.of(tx.id()), ((Result.Success<List<UUID>, DomainError>) r).value());
        assertTrue(cardRepo.findById(creditCard.id()).isEmpty());
    ...
    void setActivePersistsFlag() {
        ...
        Result<CreditCard, DomainError> r = useCase.setActive(creditCard.id(), false);

        assertTrue(r.isSuccess());
        assertFalse(((Result.Success<CreditCard, DomainError>) r).value().active());
        assertFalse(cardRepo.findById(creditCard.id()).orElseThrow().active());
    }
}

...
class AccountUseCaseTest {
    ...
    void deleteUnknownAccount() {
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(UUID.randomUUID(), new TransactionPolicy.Block());
        assertTrue(r.isFailure());
    ...
    void blockDeletesEmptyAccount() {
        ...
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(checking.id(), new TransactionPolicy.Block());
        assertTrue(r.isSuccess());
    ...
    void moveRejectsSelfTarget() {
        ...
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(account.id(), new TransactionPolicy.Move(account.id()));
        assertTrue(r.isFailure());
    ...
    void moveRejectsUnknownTarget() {
        Account account = seedChecking();
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(account.id(), new TransactionPolicy.Move(UUID.randomUUID()));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.NotFound.class, ((Result.Failure<List<UUID>, DomainError>) r).error());
    }
    ...
    void moveRejectsInactiveTarget() {
        ...
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(source.id(), new TransactionPolicy.Move(inactiveTarget.id()));
        assertTrue(r.isFailure());
        assertInstanceOf(DomainError.BusinessRule.class, ((Result.Failure<List<UUID>, DomainError>) r).error());
    }
    ...
    void purgeDeletesAccountCardsAndTransactionsButKeepsTransferSibling() {
        ...
        Result<List<UUID>, DomainError> r = useCase.deleteAccount(source.id(), new TransactionPolicy.Purge());

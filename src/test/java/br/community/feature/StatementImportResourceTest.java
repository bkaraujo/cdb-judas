...
class StatementImportResourceTest extends BaseHttpTest {
    ...
    void previewDetectsBtgIssuer() throws Exception {
        ...
        mockMvc.perform(multipart("/api/" + TEST_USER_ID + "/accounts/statement/import/preview").file(file))
    ...
    void previewEncryptedWithWrongPasswordReturns422WithWrongPasswordCode() throws Exception {
        ...
        mockMvc.perform(multipart("/api/" + TEST_USER_ID + "/accounts/statement/import/preview").file(file).param("password", "errada"))
                .andExpect(status().isUnprocessableEntity())

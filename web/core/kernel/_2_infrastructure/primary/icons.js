/* _3_infrastructure/primary/icons.js — SVG icon paths (ported verbatim from template/components/theme.jsx). */
...
window.icon = function (name, size, strokeWidth) {
  ...
  const d = window.ICONS[name] || window.ICONS.alertCircle;
  const paths = d.split('M').filter(Boolean).map(function (p) {
    return '<path d="M' + p + '"/>';
  }).join('');

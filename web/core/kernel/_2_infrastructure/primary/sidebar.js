...
(function () {
  ...
  // Fechamento contábil é da fatia accounts — kernel não pode referenciar App.ClosingService/
  // closingDialog por nome (fatia irmã). composition-root injeta o provider real depois que
  // accounts carregar; até lá o botão de fechamento fica inerte.
  let closingProvider = null;
  ...
  window.Sidebar = {
    ...
    // Injetado por composition-root depois que a fatia accounts carrega — kernel nunca
    // referencia App.ClosingService/closingDialog por nome. provider: { get(), openDialog(opts) }.
    configureClosing: function (provider) { closingProvider = provider; },
  };
})();

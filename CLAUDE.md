...
### Contextos de negócio — `br.cdb.context.*` (Hexagonal, livre de framework; DI via `Context`)
...
- **`f999`** — última fatia (pode depender de todas), sem HTTP. Cinco responsabilidades: (0) **bootstrap dos módulos** — `FeatureBootstrap` é o único bean CDI do pacote `feature`: observa o `StartupEvent` (`@Priority(1)`, depois do `DataSource` de `CoreModule`), publica no `Context` os poucos beans CDI que as engines precisam (`UserGuards`, `InternalApi`, `UserService`) e chama `initialize()` de cada `FNNNModule` na ordem da lista (f000→f999); (a) provisionamento no startup — cria o usuário `admin`/`admin` via `f000.UserService`, o que dispara em cascata o seed de categorias em `f005`; (b) **único dono do dispatch SSE** — `AccountStreamListener`/`CategoryStreamListener`/`TagStreamListener` reagem aos eventos de f002/f004/f005/f006 e chamam `SSE.dispatch`; (c) **composition root** — todo adapter que liga a porta de uma fatia ao provedor de outra vive em `f999._2_infrastructure.adapter` (hoje só `DeletionQueueAdapter`, publicado na porta pelo próprio `F999Module` — sem CDI, já que quem consome é o `WriteUseCase` Context-wired de f002); (d) **fila de retry** (fase 5) — `DeletionQueueService.runOnce()` (`F999_DELETION_QUEUE`, `@Scheduled` a cada 5min via `DeletionQueuePurgeJob`) reprocessa evento cross-slice que falhou/o processo caiu antes de completar, e recomputa `F002_BALANCE` marcado sujo.

### Plataforma — `br.cdb.core` (módulo `br-application`)
Autenticação/autorização (token opaco rotativo, `OwnershipFilter`), observabilidade (log de requisição + MDC), erro HTTP (`ProblemDetail` + `ExceptionMapper`s), config HTTP/OpenAPI, `SpaFallbackRoute` (deep-link da SPA → `index.html`), `CoreModule` (costura CDI↔`Context`: produz o `DataSource`, aplica as migrações one-shot, cria o schema e publica os adaptadores JDBC nas portas dos contextos no `StartupEvent`).
...
| **`br-commons/src/main/java/CLAUDE.md`** | **Framework comum** — índice de pacotes (`Result`, `Context`, `MessageBus`, logger, JDBC/JSON/YAML, PDF, plataforma), sem dependência de `br.cdb.*` |

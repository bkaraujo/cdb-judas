...
- **`f003`** — cartões de crédito (`/api/{uuid}/accounts/{accountId}/cards`). Extraída de `f002` (era passthrough fundido, cresceu até justificar fatia própria — ver `.claude/refactor.md`); ganhou `_0_domain`/módulo próprio na fase 2 (hospeda `CreditCard`/`CreditCardUseCase`, ex-contexto monetário, com `COD_PERSON` nativo desde a fase 3).

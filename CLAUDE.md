...
- **`monetary`** (módulo `br-context-monetary`) — lógica financeira. Facade `MonetaryContext` (acessores estáticos para os use cases); modelos `Account`, `AccountBalance`, `CostCenter`, `CreditCard`, `Statement`, `Transaction`; use cases `AccountUseCase`, `CreditCardUseCase`, `CostCenterUseCase`, `TransactionUseCase`.

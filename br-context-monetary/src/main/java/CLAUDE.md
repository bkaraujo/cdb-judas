...
├── _0_domain
│   ├── model/       Account, Balance, CostCenter, CreditCard, MonthlyBalance, Statement, Transaction
...
└── MonetaryBootstrap.java   (composition root)
```
...
* **`MonetaryBootstrap`** — `register()` monta o grafo completo (services → `BalanceRecalculationService` → use cases), assina `TransactionEventListener` no `MessageBus` e publica `MonetaryContext` no `Registry`. Chamado por `br.cdb.core.ContextBridge.monetaryContext(...)` no startup.

## Pontos não óbvios

* **`AccountEventListener` é um placeholder vazio** (`private AccountEventListener() {}`, sem métodos `@MessageListener`) — existe o pacote/classe mas nada está registrado nele. Só `TransactionEventListener` é assinado em `MonetaryBootstrap` hoje (reage a `TransactionEvents.Created/Updated/Deleted` acionando `BalanceRecalculationService`).
...
* **`Balance` vs `MonthlyBalance`** — `Balance` é o modelo de saldo "cru"; `MonthlyBalance` é a projeção por competência (`?period=yyyyMM`/`?year=yyyy`) exposta pela Facade (`getMonthlyBalance`/`getYearBalances`).
...
* **Testes** — este módulo não tem `src/test`. Toda a suíte deste contexto (unit de Service/UseCase, ArchUnit, integração HTTP) roda em `br-application/src/test/java/br/cdb/context/monetary/**` — ver `@br-application/src/main/java/CLAUDE.md`.

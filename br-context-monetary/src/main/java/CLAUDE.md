...
└── MonetaryContext.java     (Facade — acessores estáticos para os use cases)
```
...
`src/test/java/br/cdb/context/monetary/_1_application/usecase/` — cobre os 4 use cases (`AccountUseCaseTest`, `CreditCardUseCaseTest`, `CostCenterUseCaseTest`, `TransactionUseCaseTest`) com JUnit 5 puro, sem Quarkus/CDI/Mockito. `InMemoryRepositories` (mesmo pacote, visibilidade padrão) implementa as 5 portas de repositório em memória.

...
* **`br.cdb.feature.fNNN`** (f000–f010) — cada feature é um hexágono auto-contido: `_0_domain` (modelos/overlays + portas `*Repository` + eventos de domínio da feature), `_1_application` (`*Service`/`*UseCase` + commands/outcomes + `@MessageListener` best-effort), `_2_infrastructure` (`*Resource`, DTOs HTTP, `*JDBCRepository`, `FNNNModule` CDI). `f000` é a fatia-base (stream/SSE, deletion, auth, `UserGuards`, costcenter, version, closing) — todas as demais podem depender dela, ela de nenhuma feature. Ver decomposição completa em `@CLAUDE.md` (raiz), `.claude/refactor.md` (histórico da migração) e `@docs/functional_decomposition.md`.

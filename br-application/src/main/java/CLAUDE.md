...
* As funcionalidades orientadas à interface/exposição HTTP são divididas em fatias verticais numeradas sob `br.cdb.feature.fNNN` (f000–f010; ex.: `f002` = accounts, `f005` = transactions/transfer, `f008` = tags).
...
* **Testes de Arquitetura (ArchUnit):** O arquivo `br.cdb.ArchitectureTest` automatiza a validação das regras arquiteturais no pipeline. As regras principais validadas são:
  ...
  As antigas regras 7 (`user_feature_slices_must_not_depend_on_each_other`, matcher `..feature.user.(*)..`) e 8 (`auth_feature_must_not_access_user_features`) foram removidas: a migração fNNN (`.claude/refactor.md`) esvaziou `feature.user.*`, fazendo os matchers casarem zero classes (ArchUnit falha em "empty should" por padrão). A tightened replacement do plano (`fNNN` não pode depender de fatia irmã, exceto `f000`) ainda não pode ser reintroduzida: `f002↔f005`, `f007→f005`, `f008→f005` e `f010→f007` são dependências transitórias reais e deliberadas, documentadas caso a caso nas classes que as usam, pendentes da migração `ON DELETE CASCADE`.

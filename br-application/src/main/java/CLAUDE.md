...
* **`br.cdb.feature.user.*`** — features escopadas por `/api/{uuid}/…`: `accounts` (+ `balance`/`cards`/`closing`/`statement`/`transactions`/importação de extrato), `categories`, `tags`, `dashboard`, `profile`, `stream` (SSE), `deletion`. O pacote raiz contém o **`UserUseCase`** — o único use case da fatia: todos os `*Resource` de `feature.user` injetam só ele; a orquestração (use cases de contexto + serviços de feature + SSE) mora nele e o Resource faz apenas tradução de formato (HTTP ⇄ comandos/views). Ver decomposição completa em `@CLAUDE.md` (raiz) e `@docs/functional_decomposition.md`.

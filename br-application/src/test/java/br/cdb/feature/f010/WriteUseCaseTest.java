...
class WriteUseCaseTest extends AbstractUseCaseTest {
    ...
    @Test
    @DisplayName("createRule persiste a regra da pessoa")
    void createsRule() {
        val accountId = UUID.randomUUID();
        val categoryId = UUID.randomUUID();
        val costCenterId = UUID.randomUUID();

        val r = useCase.createRule(PERSON_ID, "Companhia de Saneamento", accountId, categoryId, costCenterId);
    ...
    void createsRuleNameOnly() {
        val r = useCase.createRule(PERSON_ID, "Netflix", null, null, null);
    ...
    @Test
    @DisplayName("createRule com padrão substring de regra existente → Conflict")
    void rejectsAmbiguousCreate() {
        seedRule(PERSON_ID, "Companhia de Saneamento");

        val r = useCase.createRule(PERSON_ID, "Companhia", null, null, null);
    ...
    @Test
    @DisplayName("createRule com padrão que contém regra existente (direção oposta) → Conflict")
    void rejectsAmbiguousCreateOppositeDirection() {
        seedRule(PERSON_ID, "Água");

        val r = useCase.createRule(PERSON_ID, "Companhia de Água e Saneamento", null, null, null);
    ...
    @Test
    @DisplayName("createRule ignora acento/caixa na comparação de ambiguidade")
    void ambiguityCheckIsAccentAndCaseInsensitive() {
        seedRule(PERSON_ID, "AGUA");

        val r = useCase.createRule(PERSON_ID, "Conta de Água", null, null, null);

        assertTrue(r.isFailure());
    }

    @Test
    @DisplayName("createRule não compara com regras de outra pessoa")
    void ambiguityCheckIsScopedToPerson() {
        seedRule(OTHER_PERSON_ID, "Companhia de Saneamento");

        val r = useCase.createRule(PERSON_ID, "Companhia de Saneamento", null, null, null);
    ...
    void updatesRule() {
        ...
        val r = useCase.updateRule(PERSON_ID, rule.id(), "Netflix Brasil", null, categoryId, null);
    ...
    @Test
    @DisplayName("updateRule não se autoconflita com o próprio padrão")
    void updateDoesNotConflictWithItself() {
        val rule = seedRule(PERSON_ID, "Netflix");

        val r = useCase.updateRule(PERSON_ID, rule.id(), "Netflix", null, null, null);
    ...
    void rejectsAmbiguousUpdate() {
        seedRule(PERSON_ID, "Companhia de Saneamento");
        val other = seedRule(PERSON_ID, "Netflix");

        val r = useCase.updateRule(PERSON_ID, other.id(), "Companhia", null, null, null);
    ...
    void updateUnknownRule() {
        val r = useCase.updateRule(PERSON_ID, UUID.randomUUID(), "X", null, null, null);
        assertTrue(r.isFailure());
        assertInstanceOf(BusinessError.NotFound.class, ((Result.Failure<ImportRule, BusinessError>) r).error());
    }

    @Test
    @DisplayName("deleteRule apaga a regra")
    void deletesRule() {
        val rule = seedRule(PERSON_ID, "Netflix");

...
| `Context` | Service locator manual thread-safe (`get`/`set`/`tryGet`/`remove`/`clear`) — mecanismo de DI dos contextos, sem framework. Cada `*Bootstrap` (composition root de um contexto) publica sua Facade aqui; `br.cdb.core.ContextBridge` é a única costura entre CDI e `Context`. |

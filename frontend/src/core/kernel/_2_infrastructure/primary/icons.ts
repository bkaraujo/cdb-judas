/** SVG icon paths (ported verbatim from template/components/theme.jsx). */
...
export function icon(name: string, size?: number, strokeWidth?: number): string {
  const s = size || 18;
  const sw = strokeWidth || 1.8;
  const d = ICONS[name] || ICONS.alertCircle;
  const paths = String(d)
    .split('M')
    .filter(Boolean)
    .map((p) => '<path d="M' + p + '"/>')
    .join('');
